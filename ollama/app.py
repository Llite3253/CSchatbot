...
def load_txt_data_to_history(file_path='./myfile.txt'):
    ...
    try:
        ...
        data = {line.split(":", 1)[0]: line.split(":", 1)[1].strip() for line in lines if ':' in line}
        
        keytopic = data.get('대표키워드', '')
        history = [{'role': 'system', 'content': f"{key}: {value}"} 
                   for key, value in data.items() if key != '대표키워드']
